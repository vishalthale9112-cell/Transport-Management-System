...
export default function FuelManagement() {
  ...
  const handleAdd = async (e) => {
    ...
    await createFuelLog({
      ...
      price_per_liter: Number(form.cost) / Number(form.liters),
  ...
  const totalCost = filteredLogs.reduce((sum, l) => sum + l.total_cost, 0);
          ...
          <tbody>
            {filteredLogs.map((l) => (
              <tr key={l.id}>
                ...
                <td>{l.date || "—"}</td>
                <td>{l.liters} L</td>
                <td>₹{l.total_cost.toLocaleString("en-IN")}</td>
